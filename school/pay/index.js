// var s = getApp(), r = s.requirejs("core"), d = s.requirejs("foxui"),
var e = getApp(), t = e.requirejs("core"), r = e.requirejs("foxui"), o = e.requirejs("jquery");
...
Page({
    ...
    getData: function(){
        ...
        t.post("school/info", {
            ...
            e.setData({
                ...
                alipaycode: t.alipaycode,
            })

        });
    },
    Submitpay: function(){
      var e = this, k = e.data.detail;
      e.setData({
        zhifubao: false
      })
      k['id'] = e.data.id;
      k['pay_type'] = 0;
      if(k.username == '') return t.toast('请输入姓名')
      if(k.mobile == '') return t.toast('请输入手机号')
      if(k.area == '') return t.toast('请输入详细地址')
      if(k.remark == '') return t.toast('请输入备注')
      if(k.price == '') return t.toast('请输入价格')
      t.post("school/pay", k, function(a) {
        console.log(a)
        if(a.error == 0){
        //   t.toast(a.message)
          wx.requestPayment({
    ...
    tijiao(){
        ...
        k['id'] = e.data.id;
        k['pay_type'] = 1;
        k['pay_img'] = e.data.imgs[0];
        if(k.username == '') return t.toast('请输入姓名')
        if(k.mobile == '') return t.toast('请输入手机号')
        if(k.area == '') return t.toast('请输入详细地址')
        if(k.remark == '') return t.toast('请输入备注')
        if(k.price == '') return t.toast('请输入价格')
        if(k.pay_img == '') return t.toast('请上传图片')
        t.post("school/pay", k, function(a) {
            console.log(a)
            if(a.error == 0){
              t.toast('提交成功')
